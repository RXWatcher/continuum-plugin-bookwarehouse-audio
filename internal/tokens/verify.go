...
// by the audiobooks portal. Browsers can't send Authorization headers on
// <img>/<audio> tag requests, so the portal embeds a short-TTL signed JWT in
// the URL and the byte routes validate it here.
//
...
// Audience is the expected `aud` claim shared by every audiobook backend
// plugin. The portal uses the same value when signing.
const Audience = "audiobook_backend"

// CoverFileIdx is the sentinel file_idx claim value used for cover tokens —
// covers don't address a specific file in a multi-file book.
...
//
// secret may be a base64-encoded string (preferred — matches local-audiobooks
// + the portal's storage shape) or a raw byte string; both are accepted.
...
// decodeSecret returns the HMAC key bytes. base64 is tried first; if that
// fails the raw string is returned so admins can paste either form.
