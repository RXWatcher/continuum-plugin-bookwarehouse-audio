...
// gRPC service. The plugin host invokes our gRPC service when a user request
// matches one of our manifest's http_routes; we replay the request against
// the wrapped handler and return its response.
package httproutes
...
// NewServer constructs an unconfigured server. Requests are answered with 503
