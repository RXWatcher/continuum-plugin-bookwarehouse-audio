...
// caller-provided database. The operator pre-creates the
// `bookwarehouse_audio` schema and the plugin role; the migrations apply DDL
// for the plugin's tables inside that schema.
package migrate
...
// dsn must include search_path=bookwarehouse_audio so unqualified DDL targets
