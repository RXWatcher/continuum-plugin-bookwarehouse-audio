...
<article class="panel"><div class="panel-head"><div><h2>Plugin config</h2><p class="muted">BookWarehouse connection and local direct-stream pathing live in this plugin database.</p></div><span id="config-save-state" class="badge">Loading</span></div><form id="config-form" class="config-grid"><label>Base URL<input id="cfg-base-url" name="base_url" placeholder="https://bookwarehouse.zenterprise.org"></label><label>API key<input id="cfg-api-key" name="api_key" type="password" placeholder="Leave blank to keep current key"></label><label>Default cover size<select id="cfg-cover-size" name="default_cover_size"><option value="medium">Medium</option><option value="original">Original</option><option value="thumbnail">Thumbnail</option></select></label><label class="check"><input id="cfg-direct" name="direct_file_access" type="checkbox"> Direct file access</label><div class="span-all remap-block"><div class="remap-head"><strong>Path remappings</strong><span id="remap-help" class="muted">Map BookWarehouse source paths to local container paths for direct file access.</span></div><div id="remap-list" class="remap-list"></div><button id="add-remap" type="button" class="secondary">Add remapping</button></div><button type="submit">Save config</button></form></article>
</section>
...
function createRemapRow(source,target){const row=document.createElement("div");row.className="remap-row";const sourceInput=document.createElement("input");sourceInput.className="remap-source";sourceInput.placeholder="/media/books";sourceInput.value=source||"";const arrow=document.createElement("span");arrow.className="remap-arrow";arrow.textContent="→";const targetInput=document.createElement("input");targetInput.className="remap-target";targetInput.placeholder="/mnt/books";targetInput.value=target||"";const removeButton=document.createElement("button");removeButton.type="button";removeButton.className="secondary remap-remove";removeButton.textContent="Remove";removeButton.addEventListener("click",()=>{row.remove();if(!remapList.children.length)renderRemappings([]);syncRemapControls()});row.append(sourceInput,arrow,targetInput,removeButton);return row}
function renderRemappings(remaps){remapList.innerHTML="";if(!remaps.length){const empty=document.createElement("div");empty.className="remap-empty";empty.textContent=directToggle.checked?"No remappings added yet. Add the source path from BookWarehouse and the matching local mount path.":"Enable direct file access to manage path remappings.";remapList.appendChild(empty);syncRemapControls();return}for(const remap of remaps){remapList.appendChild(createRemapRow(remap.source_path||"",remap.target_path||""))}syncRemapControls()}
function syncRemapControls(){const enabled=!!directToggle.checked;document.querySelectorAll(".remap-source,.remap-target,.remap-remove").forEach(el=>el.disabled=!enabled);addRemapButton.disabled=!enabled;remapHelp.textContent=enabled?"Map BookWarehouse source paths to local container paths for direct file access.":"Enable direct file access before adding remappings.";remapList.classList.toggle("disabled",!enabled)}
function addRemapRow(source,target){const empty=remapList.querySelector(".remap-empty");if(empty)empty.remove();remapList.appendChild(createRemapRow(source,target));syncRemapControls()}
